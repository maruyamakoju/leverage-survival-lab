...
> **「100倍レバ × 損切」は本当に勝てるのか?** — 87,325 回のモンテカルロ・バックテストで検証する個人プロジェクト
...
Binance USDT-M Perpetual の **BTC/USDT 6 年分(2020-01〜2026-05)** から 30 日窓を 500 個ランダム抽出し、5 戦略 × 7 レバレッジ水準 × 5 損切水準で **87,325 回**のシミュレーションを実行した。

**結論**:

- **100倍レバの 30 日生存率は 0%。**95% Wilson 信頼区間の上限は **0.76%**
...
| 仮説 | 結果 | 主要数値 |
|------|------|---------|
| **H1**: 100倍レバの30日生存率は損切ルール問わず < 10% | **支持(強)** | 0/25 セルが違反, **CI上限 0.76%** |
| **H2**: 各レバ倍率に最適な損切ライン(内点解)が存在 | 部分支持 | 中レバ域 (5x, 10x) で内点解、低/高レバでは端点 |
| **H3**: 戦略エッジは 10〜20倍で消失する | 測定不能 | レバ 1x でも全戦略が負の log-return(手数料負担) |
| **H4**: 50x 以上ではナイーブ戦略 vs ランダム に有意差なし | **支持** | 全 8 比較で p > 0.00625 (Bonferroni 補正後) |

詳細レポート: [`results/hypothesis_test_real_btc_n500.md`](results/hypothesis_test_real_btc_n500.md)
