...
### Cross margin × 多ポジ分散 — H1 のスコープを明確化

100倍レバが 0% 生存だったのは「**全資金を1ポジに all-in、清算→再エントリーを30日繰り返す**」というシナリオ下。
試しに `CrossMarginEngine` で「5つの独立サブ戦略 × rf=10% × 24バー保有上限」にすると:

| Lev | Cross-margin 生存率 (N=100) | 95% CI |
|----:|---------------------------:|:-------|
|  2x |   100% | [96.3%, 100%] |
|  5x |   100% | [96.3%, 100%] |
| 10x |   100% | [96.3%, 100%] |
...
**100x でも生存率 4%**(完全に死ぬわけではない)。つまり H1 の正しい解釈は「100倍レバ自体が常にダメ」ではなく、**「100倍レバ + 高頻度 all-in 再エントリーは数学的に詰む」**。
ポジション管理(分散・サイズ抑制・保有時間上限)で多少は緩和できる、ただし期待値は依然として低い。
