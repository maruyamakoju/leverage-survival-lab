...
> **「100倍レバ × 損切」は本当に勝てるのか?** — **139,996 回**のモンテカルロ・バックテストで検証する個人プロジェクト
...
Binance USDT-M Perpetual の **BTC/USDT 6 年分(2020-01〜2026-05)** から 30 日窓を **800 個**ランダム抽出し、5 戦略 × 7 レバレッジ水準 × 5 損切水準で **139,996 回**のシミュレーションを実行した。

**結論**:

- **100倍レバの 30 日生存率は 0%。** 95% Wilson 信頼区間の上限は **0.48%**(全 25 セルで違反なし)
...
| 仮説 | 結果 | 主要数値 |
|------|------|---------|
| **H1**: 100倍レバの30日生存率は損切ルール問わず < 10% | **支持(強)** | 0/25 セルが違反, **CI上限 0.48%** |
